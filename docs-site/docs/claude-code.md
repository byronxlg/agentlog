...
There are two ways to integrate agentlog with Claude Code. Both can be used together.

1. **CLAUDE.md instructions** - tell Claude to log decisions via the CLI. Simple, no setup beyond editing a file.
2. **Hook scripts** - automatically log decisions using Claude Code's hook system. Captures decisions without relying on Claude remembering to log them.
